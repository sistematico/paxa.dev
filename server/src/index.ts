...
import type { ApiResponse, PostsIndexResponse, SnippetsIndexResponse, FavoritesResponse } from 'shared/dist'
...
app.get('/api/snippets', async (c) => {
  ...
  try {
    const { snippets, snippetsByCategory, categories } = await indexSnippets(snippetsDirectory);
...
app.get('/api/bookmarks', async (c) => {
  const favoritesPath = path.join(process.cwd(), 'src', 'data', 'bookmarks.json');
  
  try {
    const { favorites, favoritesByCategory, categories } = await indexFavorites(favoritesPath);
