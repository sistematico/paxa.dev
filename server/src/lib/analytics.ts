...
export async function updatePostStats(postSlug: string): Promise<void> {
  try {
    ...
    const uniqueViewsResult = await db
      .select({ count: sql<number>`count(DISTINCT ${postViews.visitorId})` })
      .from(postViews)
      .where(eq(postViews.postSlug, postSlug))
 ...
 */
export async function updatePostReadingStats(
  postSlug: string,
  content: string
): Promise<void> {
  try {
...
export async function getSiteStats() {
  try {
    ...
    const totalHitsResult = await db
      .select({ total: sql<number>`sum(${siteVisits.totalHits})` })
      .from(siteVisits)
