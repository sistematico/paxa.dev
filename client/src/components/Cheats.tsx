...
export function PostList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Buscar tags
        const tagsResponse = await fetch(`${apiUrl}/cheats/tags/all`)
        if (tagsResponse.ok) {
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">Carregando cheats...</div>
      </div>
    )
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          {selectedTag ? (
            <>
              <p className="mb-4">
                Nenhum post encontrado com a tag "{selectedTag}".
              </p>
              <button
                type="button"
        ...
        <div key={year} className="mb-12">
          ...
          <div className="space-y-6">
            {postsByYear[year].map((post) => (
              <article
                key={post.slug}
                className="border-l-4 border-gray-700 pl-4 py-2"
              >
